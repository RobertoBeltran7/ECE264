...
long long int distance (const DataPoint * datapoint, const Centroid * centroid)
{
  ...
  long long int euclsqr;
  
  for (int i = 0; i < datapoint->dimension; i++)
    {
      euclsqr = datapoint->data[i]-centroid->data[i];
      sum += euclsqr * euclsqr; 
    }
...
int closestCentroid (int kval, DataPoint * datapoint, Centroid * *centroids)
{
  ...
  for (int i = 1; i < kval; i++)
    {
      if (mindist > distance(datapoint, centroids[i]))
	{
	  mindist = distance(datapoint, centroids[i]);
	  mindex = i;
	}
     }
...
{
  // reset all centroids
  for (int i1 = 0; i1 < kval; i1++)
    {
      Centroid_reset(centroids[i1]);
    }
  
  for (int i2 = 0; i2 < nval; i2++)
    {
      datapoints[i2]->cluster = rand() % kval;
      Centroid_addPoint(centroids[datapoints[i2]->cluster], datapoints[i2]);
    }
  
  for (int i3 = 0; i3 < kval; i3++)
    {
      Centroid_findCenter(centroids[i3]);
    }
  ...
  do
	...
	{
	  temp = datapoints[j]->cluster;
	  closest = closestCentroid(kval, datapoints[j], centroids);
	  datapoints[j]->cluster = closest;
	  if (temp == datapoints[j]->cluster)
	    flag = false;
	}
      for (int i1 = 0; i1 < kval; i1++)
	{
